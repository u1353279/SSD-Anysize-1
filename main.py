...
def pre_process(config):
    ...
    if config["perform_train_val_split"]:
    ...
    shutil.rmtree(os.path.join(config["training_path"], "Annotations"))
    shutil.rmtree(os.path.join(config["training_path"], "JPEGImages"))
...
if __name__ == "__main__":
    ...
    if CONFIG["backbone"] not in AVAILABLE_MODELS.keys():
        print("Please select from:", *AVAILABLE_MODELS.keys())
    else:

        pre_process(CONFIG)
        CONFIG["backbone_model"] =\
             AVAILABLE_MODELS[CONFIG["backbone"]](CONFIG["input_dims"])
        train(CONFIG)
